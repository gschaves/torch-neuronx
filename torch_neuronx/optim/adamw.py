...
def _single_tensor_adamw_(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable:
            assert (param.is_cuda and step_t.is_cuda) or _is_xla(
                param
            ), "If capturable=True, params and state_steps must be CUDA tensors or params must be an XLA tensor."  # noqa: E501
        ...
        if capturable:
            ...
            if amsgrad:
                ...
                # Folds in (admittedly ugly) 1-elem step_size math here to
                # avoid extra param-set-sized read+write
                # (can't fold it into addcdiv_ below because addcdiv_
                # requires value is a Number, not a Tensor)
                denom = (
